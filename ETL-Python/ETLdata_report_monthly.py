...
df[['Year', 'Month']] = df['Month/Year'].str.split('-', expand=True)# Split the 'Month/Year' column into 'Month' and 'Year' columns
...
df['Trips Per Day Shared'] = df['Trips Per Day Shared'].str.replace(',', '', regex=True).astype(float).astype('Int64')# change the dtype to int
