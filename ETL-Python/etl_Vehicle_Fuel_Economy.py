...
df = df[['Year', 'Manufacturer', 'Model', 'co2', 'co2TailpipeGpm']]
df.drop_duplicates()
...
df.rename(columns={'Manufacturer': 'Brand', 'co2': 'co2_emission_gpm'})
df = df[['Year', 'Brand', 'Model', 'co2_emission_gpm']].reset_index(drop=True)

df.drop_duplicates(subset=['Year', 'Brand', 'Model'], inplace=True, ignore_index=True)
