...
df['Service_type_id'] = df.apply(lambda r: 0 if (r['License Class'] == 'Green' or r['License Class']
                                 == 'Yellow') else 1, axis=1)  # 0 stands for 'not for hire vehicle'
...
dfKPI2 = filtered_df.groupby(['Service_type_id', 'Year', 'Month']).agg(
    {'Trips Per Day': 'sum', 'Trips Per Day Shared': 'sum', 'Unique Vehicles': 'sum'}).reset_index()

dfKPI2.rename(columns={'Trips Per Day': 'Trips_per_day', 'Trips Per Day Shared': 'Shared_trips_per_day',
              'Unique Vehicles': 'Unique_vehicles'}, inplace=True)

dfKPI2.index.name = 'Report_id'
